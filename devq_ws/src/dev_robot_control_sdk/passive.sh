...
# 检查模型文件 - 修正为model目录（单数）
if [ ! -d "model" ] || [ ! -d "model/devq" ] || [ ! -f "model/devq/policy.mnn" ]; then
...
# 发送被动模式命令
echo "发送被动模式命令..."
if rostopic pub --once /robot_control/set_mode std_msgs/Int32MultiArray "data: [0]" > /dev/null 2>&1; then
    echo "已成功发送被动模式命令到机器人"
else
    echo "错误：发送被动模式命令失败"
    exit 1
fi
